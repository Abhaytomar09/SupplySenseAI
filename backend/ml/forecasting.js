/**
 * SupplySenseAI — Pure-JS ML Forecasting Engine
 *
 * Implements three algorithms and auto-selects the best one per material
 * based on Root Mean Squared Error (RMSE) computed on a hold-out validation set.
 *
 * Algorithms:
 *  1. Linear Regression       — captures linear trend
 *  2. EWMA                    — exponentially weighted moving average (smoothing)
 *  3. Holt-Winters (Double)   — captures level + trend, best for multi-period forecast
 */
...
/** Compute Root Mean Squared Error between two equal-length arrays */
...
/**
 * Fit a linear regression (y = mx + b) to the data series.
 * Returns `{ m, b }` — slope and intercept.
 */
...
/**
 * Simple EWMA / Single Exponential Smoothing.
 * alpha: smoothing factor (0 < alpha < 1). Higher = more responsive to recent data.
 */
...
// ─────────────────────────────────────────────────────────────
// ALGORITHM 3: HOLT-WINTERS (Double Exponential Smoothing)
// ─────────────────────────────────────────────────────────────

/**
 * Double Exponential Smoothing — captures both level (L) and trend (T).
 * alpha: level smoothing, beta: trend smoothing
 * Forecast at t+h: L_t + h * T_t
 */
